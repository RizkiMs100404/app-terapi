...
<div class="min-h-screen p-6 lg:p-10 bg-[#F8FAFC]">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-10">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Registrasi Wali Murid</h1>
                <p class="text-slate-600 font-semibold italic">Menambahkan data orang tua/wali baru ke sistem</p>
            </div>
        </div>
                    ...
                    <div class="group bg-white p-8 rounded-[2.5rem] border border-indigo-50 shadow-xl shadow-indigo-900/5 relative overflow-hidden transition-all duration-500 hover:shadow-indigo-900/10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="nomor_hp_aktif" value="{{ old('nomor_hp_aktif') }}" placeholder="081234567xxx" required
                                    class="w-full bg-indigo-50/50 border-2 border-transparent rounded-2xl px-5 py-4 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100 transition-all outline-none font-medium text-slate-700">
                            </div>
                            <div class="space-y-2">
                                <label class="text-sm font-bold text-indigo-900 ml-1">Pekerjaan</label>
                                <input type="text" name="pekerjaan" value="{{ old('pekerjaan') }}" placeholder="Contoh: Karyawan Swasta"
                                    class="w-full bg-indigo-50/50 border-2 border-transparent rounded-2xl px-5 py-4 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100 transition-all outline-none font-medium text-slate-700">
                            </div>
                            <div class="md:col-span-2 space-y-2">
                                <label class="text-sm font-bold text-indigo-900 ml-1">Tahun Ajaran Aktif</label>
                                <div class="relative">
                                    {{-- Fix double icon: Pastikan appearance-none bekerja --}}
                        ...
                        <div class="space-y-6 relative z-10">
                            <div>
                                ...
                                <input type="text" name="username" value="{{ old('username') }}" required
                                    class="w-full bg-white/10 border border-white/20 rounded-2xl px-5 py-4 text-white placeholder-indigo-300 focus:bg-white/20 focus:ring-4 focus:ring-indigo-400/30 outline-none transition-all" placeholder="username_wali">
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] block mb-2 ml-1">Email Portal</label>
                                <input type="email" name="email" value="{{ old('email') }}" required
                                    class="w-full bg-white/10 border border-white/20 rounded-2xl px-5 py-4 text-white placeholder-indigo-300 focus:bg-white/20 focus:ring-4 focus:ring-indigo-400/30 outline-none transition-all" placeholder="email@contoh.com">
                            </div>
                            <div class="space-y-2">
                                ...
                                {{-- Alert Catatan Password --}}
                                <div class="flex items-start gap-2 mt-3 bg-indigo-900/30 p-3 rounded-xl border border-white/10">
                                    <svg class="w-4 h-4 text-amber-400 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
                                    <p class="text-[10px] text-indigo-100 leading-relaxed font-medium">
                                        <span class="text-amber-400 font-bold uppercase tracking-wider">Penting:</span>
                                        Gunakan minimal 8 karakter dengan kombinasi huruf dan angka agar akun tetap aman.
                                    </p>
                                </div>
                            </div>
                        </div>
...
<style>
    /* Menghilangkan icon default dropdown di beberapa browser (Chrome/Safari) */
    select {
