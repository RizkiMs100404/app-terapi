        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm mb-8">
            <form action="{{ route('jadwal-terapi.index') }}" method="GET" class="flex flex-col lg:flex-row gap-3">
                ...
                <div class="flex flex-wrap lg:flex-nowrap gap-3">
                    {{-- Filter Tingkat --}}
                    <select name="tingkat" class="px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold text-slate-700 outline-none cursor-pointer shadow-inner focus:ring-2 focus:ring-indigo-100">
                    ...
                    <input type="date" name="tanggal" value="{{ $tanggalAktif }}" class="px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold text-slate-700 outline-none cursor-pointer shadow-inner focus:ring-2 focus:ring-indigo-100">
                    ...
                    @if(request()->anyFilled(['search', 'hari', 'tanggal', 'tingkat']))
            ...
            <div class="relative group">
                ...
                <div class="relative bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-indigo-100/50 transition-all duration-500 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        {{-- Nama Siswa, Tingkat & Kelas --}}
                        <div class="mb-6">
                            <h3 class="text-xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors leading-tight">
                                {{ $j->siswa->nama_siswa }}
                            </h3>
                            <p class="text-[10px] font-bold text-indigo-500 uppercase tracking-wider mb-3 mt-1">
                        ...
                        <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100 group-hover:bg-white group-hover:shadow-md transition-all duration-300">
                            <div class="relative">
                                @if($j->guru && $j->guru->foto)
                                    <img src="{{ asset('storage/foto_guru/' . $j->guru->foto) }}" class="w-10 h-10 rounded-xl object-cover shadow-md border border-white" alt="Foto">
                                @else
                        ...
                        <div class="flex items-center gap-2 mt-6 pt-5 border-t border-slate-50">
                            <a href="{{ route('jadwal-terapi.show', $j->id) }}" class="flex-1 py-3 bg-slate-50 border border-slate-200 text-slate-600 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all text-center">
                                DETAIL
                            </a>
                            
                            <a href="{{ route('jadwal-terapi.edit', $j->id) }}" class="flex-1 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all text-center shadow-sm">
                                EDIT
                            </a>

                            <form action="{{ route('jadwal-terapi.destroy', $j->id) }}" method="POST" class="flex-none">
                                @csrf @method('DELETE')
                                <button type="submit"
                                        onclick="return confirm('Hapus jadwal {{ $j->siswa->nama_siswa }}?')"
                                        class="px-4 py-3 bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl transition-all flex items-center justify-center border border-rose-100 hover:border-rose-500 shadow-sm">
