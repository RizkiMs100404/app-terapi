            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-[3rem] p-8 md:p-12 border border-indigo-50 shadow-xl shadow-indigo-900/5 relative overflow-hidden group">
                    ...
                    <div class="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
                        {{-- Avatar --}}
                        <div class="relative">
                            <div class="w-32 h-32 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[2.5rem] flex items-center justify-center text-white text-5xl font-black shadow-2xl shadow-indigo-200 uppercase">
                                {{ substr($guru->user->name, 0, 1) }}
                            </div>
                            @if($guru->status_kerja == 'Aktif')
                            <div class="absolute -bottom-2 -right-2 w-10 h-10 bg-emerald-500 border-4 border-white rounded-full flex items-center justify-center text-white shadow-lg" title="Status Aktif">
                        ...
                        <div class="text-center md:text-left flex-1">
                            ...
                            <div class="flex items-center justify-center md:justify-start gap-4 text-slate-500 font-bold text-sm italic">
                                <span class="flex items-center gap-2">
                                    <svg class="w-4 h-4 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 00-2 2z"></path></svg>
                    ...
                    <div class="mt-12 pt-10 border-t border-slate-50 relative z-10">
                        <p class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-5 ml-1">Spesialisasi & Keahlian Terapi</p>
                        <div class="flex flex-wrap gap-3">
                            @forelse($guru->keahlian_terapi ?? [] as $skill)
                                <div class="px-5 py-3 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center gap-3 group/skill hover:bg-indigo-600 transition-all duration-300 cursor-default">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm group hover:shadow-xl transition-all">
                        ...
                        <p class="text-xs text-emerald-500 font-bold mt-2 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span> WhatsApp Aktif
                        </p>
                    </div>
                    ...
                    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm group hover:shadow-xl transition-all">
                        ...
                        <p class="text-xs text-slate-400 font-bold mt-2 italic uppercase">Terdaftar di Sistem</p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden group">
                    ...
                    <div class="space-y-6 relative z-10">
                        <div class="bg-white/5 p-4 rounded-2xl border border-white/5 group-hover:border-indigo-500/30 transition-colors">
                            <p class="text-slate-500 text-[10px] font-black uppercase mb-1 tracking-widest">Username</p>
                            <p class="text-lg font-bold tracking-tight text-indigo-100">{{ $guru->user->username }}</p>
                        </div>
                        <div class="bg-white/5 p-4 rounded-2xl border border-white/5 group-hover:border-indigo-500/30 transition-colors">
                            <p class="text-slate-500 text-[10px] font-black uppercase mb-1 tracking-widest">Email Terdaftar</p>
                            <p class="text-lg font-bold tracking-tight text-indigo-100 break-all">{{ $guru->user->email }}</p>
                        </div>
                ...
                <div class="p-8 bg-white rounded-[2.5rem] border border-indigo-50 shadow-xl shadow-indigo-900/5 relative overflow-hidden group">
                    <div class="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            ...
                            <p class="text-[9px] font-bold text-slate-400 leading-relaxed uppercase mt-1">Sesuai standar operasional SLB Negeri Bagian B Garut.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-8 py-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                    <div class="space-y-3">
                        <div class="flex justify-between items-center text-[10px] font-bold uppercase tracking-tighter">
                            ...
                            <span class="text-slate-600">{{ $guru->created_at->format('d M Y') }}</span>
                        </div>
                        <div class="flex justify-between items-center text-[10px] font-bold uppercase tracking-tighter">
                            <span class="text-slate-400 italic">Update Terakhir</span>
                            <span class="text-slate-600">{{ $guru->updated_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
            </div>
