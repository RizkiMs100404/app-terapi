...
<div class="lg:col-span-4 space-y-6">
    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/50 text-center relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="inline-flex p-1 bg-white rounded-full shadow-xl mb-4">
                <div class="w-32 h-32 bg-slate-100 rounded-full flex items-center justify-center overflow-hidden border-4 border-white">
                    {{-- Tampilan Foto Asli (Anti Gepeng) --}}
                    @if($siswa->foto)
                    ...
                    @else
                        ...
                        @php 
                            $bg = $siswa->jenis_kelamin == 'L' ? '4f46e5' : 'ec4899';
                        @endphp
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($siswa->nama_siswa) }}&background={{ $bg }}&color=fff&size=128" 
                             class="w-full h-full object-cover" 
                             alt="Avatar">
                    @endif
                </div>
            ...
            <p class="text-indigo-600 font-bold mt-1 uppercase tracking-widest text-[10px]">NIS: {{ $siswa->nis }}</p>

            {{-- Badge Status, Tingkat & Kelas --}}
            <div class="flex flex-wrap items-center justify-center gap-2 mt-4">
                <span class="px-4 py-1.5 bg-indigo-600 text-white rounded-full text-[10px] font-black uppercase tracking-tighter shadow-md shadow-indigo-100">
                    {{ $siswa->tingkat }}
                </span>
                <span class="px-4 py-1.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-full text-[10px] font-black uppercase tracking-tighter">
                    Kelas {{ $siswa->kelas }}
                </span>
                <span class="px-4 py-1.5 bg-slate-100 text-slate-600 rounded-full text-[10px] font-black uppercase tracking-tighter">
                    TA {{ $siswa->tahunAjaran->rentang_tahun ?? '-' }}
                </span>
            </div>
        </div>
        ...
        <div class="mt-8 pt-8 border-t border-slate-100 grid grid-cols-2 gap-4">
            <div class="text-left bg-slate-50 p-3 rounded-2xl">
    ...
    <div class="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <p class="text-lg font-black leading-none">{{ $siswa->orangtua->user->name ?? 'N/A' }}</p>
