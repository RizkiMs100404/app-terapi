...
<div class="min-h-screen p-4 lg:p-10 bg-[#FBFEFD]">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
            <div>
                <a href="{{ route('guru.jadwal.index') }}" class="group inline-flex items-center text-emerald-600 font-bold mb-3 transition-all hover:text-emerald-700">
                ...
                <h1 class="text-4xl font-black text-emerald-950 tracking-tight">Laporan Sesi <span class="text-emerald-600"> {{ $nomorSesi }}</span></h1>
                <p class="text-slate-500 font-medium">Menginput hasil terapi untuk <span class="text-emerald-700 font-bold">{{ $jadwal->siswa->nama_siswa }}</span></p>
            </div>
        ...
        <form action="{{ route('guru.rekam-terapi.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            <input type="hidden" name="id_jadwal" value="{{ $jadwal->id }}">
            <input type="hidden" name="nomor_sesi" value="{{ $nomorSesi }}">

            {{-- Main Form (Left) --}}
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-emerald-900/5 border border-white">
                    <label class="block text-lg font-black text-emerald-900 mb-6">Bagaimana kemajuan siswa sesi ini?</label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach(\App\Models\RekamTerapi::listKemajuan() as $key => $label)
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="hasil_kemajuan" value="{{ $key }}" class="hidden peer" required {{ $loop->iteration == 2 ? 'checked' : '' }}>
                            <div class="p-5 rounded-[1.5rem] border-2 border-emerald-50 bg-emerald-50/30 peer-checked:bg-emerald-600 peer-checked:border-emerald-600 peer-checked:text-white transition-all duration-300 group-hover:border-emerald-200">
                    ...
                    <div class="mt-8 space-y-6">
                        <div>
                            ...
                            <textarea name="catatan_terapis" rows="4" required placeholder="Tuliskan detail aktivitas dan respon siswa..."
                                class="w-full rounded-[1.5rem] border-none bg-slate-50 focus:ring-2 focus:ring-emerald-500 p-6 text-slate-700 font-medium"></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-black text-emerald-900 mb-2 ml-1">Rekomendasi Selanjutnya</label>
                            <textarea name="rekomendasi_lanjutan" rows="3" placeholder="Saran untuk sesi atau latihan di rumah..."
                                class="w-full rounded-[1.5rem] border-none bg-slate-50 focus:ring-2 focus:ring-emerald-500 p-6 text-slate-700 font-medium"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-emerald-900/5 border border-white">
                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-black text-emerald-900 mb-2 uppercase tracking-widest text-[10px]">Tanggal Pelaksanaan</label>
                            <input type="date" name="tanggal_pelaksanaan" value="{{ date('Y-m-d') }}" required
                                class="w-full rounded-2xl border-none bg-slate-50 focus:ring-2 focus:ring-emerald-500 font-bold text-emerald-800 p-4">
                        </div>

                        <div>
                            <label class="block text-sm font-black text-emerald-900 mb-2 uppercase tracking-widest text-[10px]">Skor Grafik (0-100)</label>
                            <input type="number" name="skor_grafik" placeholder="0" min="0" max="100" required
                        ...
                        {{-- Dokumentasi Sesi Section with Preview --}}
                        <div>
                            ...
                            <div class="relative group">
                                <label for="file_dokumentasi" class="flex flex-col items-center justify-center w-full h-48 border-4 border-dashed border-emerald-50 hover:bg-emerald-50 hover:border-emerald-200 transition-all rounded-[2rem] cursor-pointer bg-slate-50/50 overflow-hidden">
                                    ...
                                    <div id="placeholder-content" class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Klik Untuk Upload</p>
                                    </div>
                                    <input type="file" name="file_lampiran" id="file_dokumentasi" class="hidden" accept="image/*" onchange="previewImage(this)" />
                                </label>
                            </div>
                        </div>
                    </div>
