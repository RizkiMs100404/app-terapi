...
<div class="min-h-screen p-4 lg:p-10">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
            <div>
                <h1 class="text-4xl font-black text-emerald-900 tracking-tight">Jadwal <span class="text-emerald-500">Terapi</span></h1>
                <p class="text-slate-700/70 mt-2 font-medium">Berikut adalah agenda terapi siswa Anda minggu ini.</p>
            </div>

            <form action="{{ route('guru.jadwal.index') }}" method="GET" class="bg-white p-2 rounded-[2.5rem] shadow-xl shadow-emerald-900/5 border border-emerald-100 flex flex-col xl:flex-row gap-2">
                {{-- Search --}}
                <div class="relative flex-1">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama..."
                        class="w-full xl:w-48 border-none bg-transparent py-3 pl-12 pr-4 focus:ring-0 text-sm font-semibold text-emerald-900">
                ...
                <div class="h-8 w-[1px] bg-emerald-100 hidden xl:block mt-2"></div>

                {{-- Filter Tingkat --}}
                <select name="tingkat" class="border-none bg-transparent py-3 px-4 focus:ring-0 text-sm font-bold text-emerald-700 cursor-pointer">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            @forelse ($jadwal as $item)
                ...
                <div class="group bg-white rounded-[2.5rem] p-8 shadow-sm border {{ $item->hari == $hariIni ? 'border-emerald-500 ring-2 ring-emerald-500/10' : 'border-emerald-50' }} hover:shadow-2xl hover:shadow-emerald-200/50 transition-all duration-500 relative overflow-hidden">
                    ...
                    {{-- Badge Status --}}
                    @if($laporanSelesai)
                        <div class="absolute -right-12 top-6 bg-amber-500 text-white text-[10px] font-black py-1 w-40 text-center rotate-45 shadow-lg z-10">
                            SELESAI
                        </div>
                    @elseif($isRealtimeToday && $item->hari == $hariIni)
                        <div class="absolute -right-12 top-6 bg-emerald-500 text-white text-[10px] font-black py-1 w-40 text-center rotate-45 shadow-lg z-10">
                            HARI INI
                        </div>
                    @endif
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded text-[9px] font-black border border-emerald-100 uppercase tracking-tighter">
                        ...
                        <div class="flex items-center gap-2 mb-8">
                            ...
                            <p class="text-xs font-bold {{ ($isRealtimeToday && $item->hari == $hariIni) ? 'text-emerald-600' : 'text-slate-400' }}">
                                {{ $laporanSelesai ? 'Laporan Telah Terisi' : (($isRealtimeToday && $item->hari == $hariIni) ? 'Sesi Terapi Aktif' : 'Terjadwal: ' . $item->hari) }}
                            </p>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <a href="{{ route('guru.jadwal.show', $item->id) }}"
                               class="flex items-center justify-center gap-2 w-full py-4 bg-white text-emerald-600 border-2 border-emerald-100 font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-emerald-50 hover:border-emerald-200 transition-all duration-300">
            ...
            @empty
                <div class="col-span-full flex flex-col items-center justify-center py-32 bg-white rounded-[3rem] border-4 border-dashed border-emerald-100">
                    ...
                    <h3 class="text-2xl font-black text-emerald-900">Jadwal Tidak Ditemukan</h3>
                    <p class="text-emerald-600/60 font-medium mt-2">Coba sesuaikan filter atau pencarian Anda.</p>
